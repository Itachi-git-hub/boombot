...
MIN_TRADE_INTERVAL = 120  # secondes
SPIKE_THRESHOLD = 15
...
def place_trade(ws, contract_type):
    ...
    if datetime.utcnow() - last_trade_time < timedelta(seconds=MIN_TRADE_INTERVAL):
        print("⏱ Délai non respecté. Trade ignoré.")
        return

    if daily_loss >= STOP_LOSS_DAILY:
        print("🛑 Stop Loss journalier atteint.")
        send_telegram("🛑 Stop Loss journalier atteint. Plus de trades aujourd'hui.")
        return
        ...
        if data["proposal_open_contract"].get("is_expired"):
            ...
            if profit > 0:
                ...
                if current_level < MARTINGALE_LEVELS:
                    ...
                    send_telegram("⚠ Échec martingale. Reprise à zéro.")
                    current_level = 0
                    current_stake = STAKE_INIT
